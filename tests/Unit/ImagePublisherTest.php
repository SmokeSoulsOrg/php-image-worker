...
class ImagePublisherTest extends TestCase
{
    ...
    public function test_it_publishes_message_to_configured_queue(): void
    {
        $payload = [
            ...
            'local_path' => 'pornstar-images/image.jpg'
        ];

        Config::set('services.rabbitmq.image_update_queue', 'image-update');

        $mockChannel = Mockery::mock(AMQPChannel::class);
        $mockChannel->shouldReceive('queue_declare')->once()->with('image-update', false, true, false, false);
        $mockChannel->shouldReceive('basic_publish')->once()->with(Mockery::on(function ($msg) use ($payload) {
